...
// All four arguments are required; callers that want a default
// git-based VCSOperations should call vcs.NewGitOps(cfg, gitcmd) (or
// the factory vcs.NewVCSOperations) and pass the result.
...
// checkStackUsable returns true if spr can safely operate on the current stack.
// If the VCS reports a problem (e.g. multi-head ambiguity in jj mode), prints
// the message as an error and returns false. There is no prompt — the failure
// cases that survive auto-revset have no safe "continue anyway" answer.
func (sd *stackediff) checkStackUsable() bool {
...
func (sd *stackediff) EditCommitDone(ctx context.Context, update bool) {
	...
	if errors.Is(err, vcs.ErrRebaseConflict) {
		// Preserve pre-refactor UX: the original spr.go printed this
		// exact recovery hint, and users (and any external docs) rely
		// on the wording.
		fmt.Fprintf(sd.output, "Rebase conflict detected. Resolve conflicts and run '%s edit --done' again.\n",
			sd.vcsOps.CommandName())
		return
	}
	if err != nil {
...
func (sd *stackediff) UpdatePullRequests(ctx context.Context, reviewers []string, count *uint) {
	...
	rawLocal, err := sd.vcsOps.GetLocalCommitStack()
	if err != nil {
		fmt.Fprintf(sd.output, "error: %s\n", err)
		return
	}
	localCommits := alignLocalCommits(rawLocal, githubInfo.PullRequests)
