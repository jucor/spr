...
Auto-abandon is parity with git's silent patch-id drop: it happens on
every `spr update` without user opt-in. We do diverge from git mode
in one place — `identifyOrphanChangeIDs` writes a user-visible notice
naming each abandoned commit by PR number and subject, plus a
recovery hint:

```
Detected 3 PR(s) closed-not-merged on GitHub whose commits are in
your local stack. These were absorbed by an upstream squash-merge.
Abandoning local copies to avoid rebase conflicts:
  - PR #2509: Add SKIP_GOLDEN env var
  - PR #2510: Speed up CI: uv pip
  - PR #2512: Per-discrepancy test infrastructure
If this looks wrong, run `jj op log` and `jj op restore <id>` to undo.
```

The git equivalent (silent patch-id drop) doesn't expose this
information because git has no introspection equivalent of `jj op
log`. Since jj does, we surface it.
...
| Method | Change |
|---|---|
| `FetchAndRebase(cfg, orphanChangeIDs)` | **Signature change** — added `orphanChangeIDs []string` for the cascade-orphan abandon path (no-op in git mode) |
| `AbandonChangeIDs(changeIDs)` | **New** — `jj abandon <id>` in jj mode, no-op in git mode |
...
- **Multi-commit PRs.** Deferred to a separate branch; not yet merged
  into `jj-compat`. spr remains one-commit-per-PR in both modes.
