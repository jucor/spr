...
func TestJjOpsGetLocalCommitStack_AllHaveIDs(t *testing.T) {
	...
	commits, err := ops.GetLocalCommitStack()
	require.NoError(t, err)
...
func TestJjOpsGetLocalCommitStack_ImmutableCommitError(t *testing.T) {
	...
	_, err := ops.GetLocalCommitStack()
	require.Error(t, err, "expected error for immutable commit")
	msg := err.Error()
...
// --- GetLocalCommitStack: log call failures ---
//
// jj log is expected to always succeed in well-formed repos; if it
// fails, JjOps returns the wrapped underlying error so the spr layer
// can surface it cleanly. These tests pin the error-return behavior
// so a future change doesn't silently swallow the failure.
...
func TestJjOpsGetLocalCommitStack_FirstLogFails(t *testing.T) {
	...
	_, err := ops.GetLocalCommitStack()
	require.Error(t, err, "must return error on first jj log failure")
	assert.Contains(t, err.Error(), "revset parse error")
}
...
func TestJjOpsGetLocalCommitStack_SecondLogFails(t *testing.T) {
	...
	_, err := ops.GetLocalCommitStack()
	require.Error(t, err, "must return error on second jj log failure")
	assert.Contains(t, err.Error(), "repo went sideways")
}
...
// result, the documented behavior is to return an error — we'd never expect
// this in practice but pin it so an accidental silent-return regression is
// caught.
...
func TestJjOpsGetLocalCommitStack_PostDescribeReparseFails(t *testing.T) {
	...
	_, err := ops.GetLocalCommitStack()
	require.Error(t, err, "expected error on re-parse failure")
	assert.Contains(t, err.Error(), "unable to add commit-id trailers")
}
