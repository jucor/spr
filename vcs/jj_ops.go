...
// Returns an error rather than panicking so the spr layer can surface
// a clean message (the immutable-commit case is the realistic failure
// — it has a known fix-it recipe documented in the error string).
func (j *JjOps) GetLocalCommitStack() ([]git.Commit, error) {
	...
	if !valid {
		...
		for i, p := range parsed {
			if p.sprCommitID == "" && !p.empty {
				...
				newDesc += "\n\ncommit-id:" + newID
				describeErr := j.jjcmd.JjArgs([]string{"describe", "-r", p.changeID, "-m", newDesc}, nil)
				if describeErr != nil {
					return nil, fmt.Errorf(
						"cannot add commit-id trailer to %s (%s) — commit is likely immutable\n\n"+
							...
							"Underlying error: %w",
						p.changeID, p.subject, p.changeID, describeErr)
				}
		...
		err = j.jjcmd.JjArgs([]string{"log", "--no-graph", "--reversed", "--color=never", "-r", "trunk()..(@:: | ::@)", "-T", template}, &output)
		if err != nil {
			return nil, fmt.Errorf("jj log (after describe): %w", err)
		}
		reparsed, revalid := parseJjLogOutput(output)
		if !revalid {
			return nil, fmt.Errorf("unable to add commit-id trailers via jj describe")
		}
		parsed = reparsed
	}
