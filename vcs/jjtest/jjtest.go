...
func NewRepo(t *testing.T) Repo {
	...
	// Isolate user-global jj/git config so a developer's local
	// ~/.config/jj/config.toml (custom revset aliases, ui templates,
	// signing config) can't leak into tests and silently change
	// template output or behavior. The per-repo `--repo` config set
	// below covers identity/editor/signing but jj's revset aliases
	// resolve at user level too. t.Setenv auto-restores at test end.
...
// uniqueToken returns a short hex string unique across all calls in a
// process. Used for unique filenames and synthetic commit-id trailers
// inside a single test. Atomic so any future t.Parallel() use stays
// safe without race-detector hits.
var tokenCounter atomic.Int64

func randomToken(t *testing.T) string {
	t.Helper()
	return fmt.Sprintf("%08x", tokenCounter.Add(1))
}
